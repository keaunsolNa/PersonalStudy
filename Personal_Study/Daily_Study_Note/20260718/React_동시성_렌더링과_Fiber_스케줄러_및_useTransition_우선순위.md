...
`isPending` 의 정체는 자체 관리되는 boolean state 다. `startTransition` 은 콜백 전에 `setPending(true)` 를 **트랜지션보다 높은 우선순위로** 커밋하고, 트랜지션 렌더가 완료되는 커밋에서 `setPending(false)` 를 반영한다. 즉시 true 가 되지만 false 복귀는 트랜지션 커밋에 묶인다. 이 훅을 쓰는 컴포넌트는 매번 리렌더되므로 소비 범위를 작게 잘라둬야 한다.
