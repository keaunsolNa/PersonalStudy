...
TypeScript의 모듈 해석 전략은 2015년 `classic`에서 시작해 2026년 현재 `bundler`까지 네 단계로 발전했다. `classic`은 TS 1.x 초창기 ambient module 시대의 잔재로, 상대 경로는 파일 시스템을 그대로 따라가지만 비상대 경로는 단순히 컴파일 루트에서 한 단계씩 위로 올라가며 `.ts`/`.d.ts`만 찾는다. `node_modules`를 모르기 때문에 npm 생태계가 등장한 이후로는 사실상 사용하지 않는다. 2026년 TS 5.x에서 `module: amd`/`system`/`umd`일 때만 기본값으로 잠깐 등장한다.
...
ESM 파일에서 상대 경로 import에 `.js`를 강제하는 이유는 Node.js의 ECMAScript loader가 확장자 자동 추론을 하지 않기 때문이다. TypeScript는 컴파일 결과 `.js` 파일에서 해당 import가 그대로 동작해야 하므로, 소스에서도 미리 `.js`로 쓰도록 요구한다. IDE 자동완성은 `.ts` 파일을 가리키더라도 import 문자열은 `.js`로 남는다. 이 동작이 어색하지만 ESM 표준의 결과다.

```typescript
// .cts 파일에서는 require/module.exports 사용 가능
...
`bundler` 옵션은 Vite, webpack 5, esbuild, Rspack, Turbopack 같은 번들러가 자체 resolver를 갖는다는 전제로 만든 lax 모드다. Node ESM의 엄격함 중 두 가지를 완화한다. 첫째, 상대 import에 `.js` 확장자를 강제하지 않는다. 둘째, `package.json` `"type"` 필드와 무관하게 `.ts` 파일을 항상 ESM처럼 다룬다. 대신 conditional exports는 그대로 평가하므로, 라이브러리가 `import`/`require` 분기를 두면 번들러 설정과 동일한 조건을 본다.
...
// tsconfig.json — Next.js 14+ / Vite 5 권장
{
  "compilerOptions": {
    "target": "ES2022",
    "module": "preserve",      // bundler와 짝
    "moduleResolution": "bundler",
    "esModuleInterop": true,
    "allowImportingTsExtensions": false,
    "resolveJsonModule": true,
    "isolatedModules": true,
    "noEmit": true,            // 번들러가 transpile 담당
    "jsx": "preserve"
  }
}
```

`module: "preserve"`는 TS 5.4에서 추가된 값으로, import/export 구문을 그대로 보존해 번들러에 위임한다. `moduleResolution: "bundler"`와 짝을 이룬다. 만약 `module: "esnext"`로 설정하면 동작은 비슷하지만, dynamic `import()` 평가 시 미묘하게 다른 결과가 나올 수 있다.

trade-off는 명확하다. `bundler` 모드는 번들러 없이 Node로 직접 실행하면 import가 깨진다. 따라서 라이브러리를 빌드해 npm에 발행할 때는 절대 사용하지 않는다. 라이브러리는 `nodenext`, 애플리케이션은 `bundler`로 나누는 것이 정석이다.

## 4. paths와 baseUrl — 컴파일러 차원 별칭

`paths`는 TypeScript가 모듈 식별자를 다시 매핑하는 컴파일러 차원의 기능이다. 런타임에는 어떤 효과도 없다. `tsc`가 출력하는 `.js`에는 `paths`가 적용된 결과가 그대로 남지 않고, 원본 경로가 그대로 남는다(에디터 자동완성과 타입 체크만 보정).
...
이 설정으로 `import { Logger } from "@/lib/logger"`를 쓰면 타입은 잘 잡히지만, 런타임에서는 Node가 `@/lib/logger`라는 패키지를 찾으려 시도해 실패한다. 따라서 번들러에 동일한 alias를 동기화해야 한다.
...
`exports` 필드는 Node 12.7부터 안정화된 라이브러리 진입점 명세다. `main`/`module`/`browser` 필드를 한꺼번에 대체하며, 외부에서 접근 가능한 서브패스를 캡슐화한다. `exports`가 선언된 패키지는 명시되지 않은 내부 경로(`my-lib/internal/private`)로 import할 수 없다.
...
conditional key는 위에서 아래로 평가되며 첫 매치를 반환한다. `types`는 **반드시 가장 위**에 두어야 하는데, TypeScript가 다른 조건보다 먼저 매치되면 타입 파일을 찾지 못하기 때문이다. 이는 TS 4.7 릴리스 노트와 Node 공식 문서가 모두 강조하는 규칙이다.

subpath pattern `./icons/*`는 와일드카드 매핑이다. `my-lib/icons/star`를 import하면 `./dist/icons/star.js`로 해석된다. `typesVersions`(구식 방법)는 더 이상 권장하지 않으며, `exports`의 `types` 조건으로 통합한다.

`bundler`와 `nodenext`는 모두 `exports`를 평가하지만, `bundler`는 `browser` 조건을 추가로 매치하는 반면 `nodenext`는 기본적으로 `node`/`import`/`require`만 본다. webpack은 `resolve.conditionNames`로 조건 우선순위를 바꿀 수 있다.

## 6. module과 moduleResolution 호환 매트릭스 (TS 5.x)

`module`과 `moduleResolution`은 임의로 조합되지 않는다. TS 5.x는 잘못된 조합에 경고를 띄우거나 컴파일 에러를 낸다.
...
| `amd`/`umd`/`system` | `classic`, `node10` | 각 포맷 | 브라우저 레거시 |

`module: "nodenext"`로 잡으면 자동으로 ESM 규칙이 적용된다. import에 `.js` 확장자가 없으면 `TS2835` 에러, `.cts`에서 top-level `import`를 쓰면 `TS1259` 에러가 난다. 반대로 `module: "esnext"` + `moduleResolution: "node10"`은 합법이지만 `exports` 필드를 못 보기 때문에 모던 라이브러리에서 진입점이 어긋난다.
...
pnpm/yarn workspaces 기반 모노레포에서는 `paths`와 project references를 함께 쓴다. workspace 의존성(`"@org/shared": "workspace:*"`)은 pnpm이 symlink로 해결하므로, TypeScript는 그 symlink를 따라가면서 타입을 본다. 단, IDE 응답 속도와 빌드 격리를 위해 project references를 권장한다.
...
project references와 `paths`가 충돌하면 TS 5.x는 references 결과를 우선한다. 라이브러리 측 `package.json`의 `exports`도 함께 정의해 두면, 외부 소비자(별도 모노레포 밖)와 내부 alias 양쪽에서 동일한 타입을 본다.
...
// dual-publish 패키지의 package.json
{
  ...
  "version": "2.0.0",
  "type": "module",
  "main": "./dist/index.cjs",        // 구식 소비자 폴백
  "module": "./dist/index.js",       // 구식 번들러 힌트
  "types": "./dist/index.d.ts",      // 구식 폴백
  "exports": {
    ".": {
      "types": {
...
types-conditional은 두 가지 배치가 있다. 단순한 경우 최상위에 `"types": "./dist/index.d.ts"` 하나만 둔다. ESM/CJS 양쪽에서 타입이 달라야 하면 위 예제처럼 `types` 안에 다시 `import`/`require`를 중첩한다. 두 번째 형태가 필요한 이유는 ESM `.d.ts`에서 `export = ` 같은 CJS 전용 문법을 쓸 수 없기 때문이다. TS 5.0부터 두 형태 모두 안정 지원된다.
